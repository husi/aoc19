...
def needed(amount):
    remainder = {}
    amount, _ = calc_needded_ORE(amount, 'FUEL', recipes, remainder)
    return amount
...
if __name__ == '__main__':
    ...
    print('For one: {}'.format(for_one))

    amount = 0
    used = 0

    while used < target:
        increase = (target - used) // for_one + 1
        amount += increase
        used = needed(amount)
        print('For amount: {} needed: {}'.format(amount, used))
